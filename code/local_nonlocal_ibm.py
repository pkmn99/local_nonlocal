...
def save_data():
    ...
    ra_c =(ts_c-tair_c)/h_c * 1.2250 * 1003.5
    bo_c = h_c/le_c
    lam_c = 1/(4*B*tair_c**3)
    F_c = (1.2250 * 1003.5 *lam_c)/ra_c*(1+1/bo_c)
    ...
    local=dts.where((dts>ts_diff.quantile(0.001))&(dts<ts_diff.quantile(0.999))).mean(dim='time')
