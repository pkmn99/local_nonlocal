...
def make_plot():
    ...
    ds_r[var].plot(ax=ax1,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)
    
    ax2 = fig.add_subplot(3, 2, 2, projection=ccrs.PlateCarree())
    dsoff_r[var].plot(ax=ax2,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)
    ...
    [a.coastlines() for a in [ax1,ax2,ax3,ax4,ax5]]
    [a.set_extent([-180, 180, -60, 80]) for a in [ax1,ax2,ax3,ax4,ax5]]
    ...
    # Add colorbar
    cbar1_pos = [ax4.get_position().x0, ax4.get_position().y0-0.04,
                 ax4.get_position().width*1.0, 0.015]
    ...
    plt.savefig('../figure/local_impact_comparision20230227.png',dpi=300,bbox_inches='tight')
    print('fig saved')
