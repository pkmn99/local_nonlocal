...
def make_plot():
    ...
    ds[var].plot(ax=ax1,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)
    
    ax2 = fig.add_subplot(3, 2, 2, projection=ccrs.PlateCarree())
    dsoff[var].plot(ax=ax2,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)
    
    ax3 = fig.add_subplot(3, 2, 3, projection=ccrs.PlateCarree())
    di[var].plot(ax=ax3,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)
    
    ax4 = fig.add_subplot(3, 2, 4, projection=ccrs.PlateCarree())
    doff[var].plot(ax=ax4,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)
    
    ax5 = fig.add_subplot(3, 2, 5, projection=ccrs.PlateCarree())
    dibm[var].plot(ax=ax5,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)

    ax6 = fig.add_subplot(3, 2, 6, projection=ccrs.PlateCarree())
    dibm['Tair_nonlocal'].plot(ax=ax6,cmap='RdBu_r',vmin=-2,vmax=2,add_colorbar=False)
    ...
    ax2.set_title('Offline subgrid (residue)') 
    ax3.set_title('Checkerboard')
    ax4.set_title('Offline (residue)')
    ax5.set_title('IBM (residue)')
    ax6.set_title('IBM (Tair)')
    
    # Add colorbar
    cbar1_pos = [ax6.get_position().x0, ax6.get_position().y0-0.04, 
                 ax6.get_position().width*1.0, 0.015]
    
    cax1 = fig.add_axes(cbar1_pos)
    cb1 = mpl.colorbar.ColorbarBase(ax=cax1, cmap='RdBu_r', norm=Normalize(vmin=-2, vmax=2),
                                    orientation='horizontal', ticks=np.arange(-2, 2.1, 0.5))
    ...
    ax6.text(-0.06, 1.01, 'f', fontsize=14, transform=ax6.transAxes, fontweight='bold')
    
    plt.savefig('../figure/nonlocal_impact_map20230227.png',dpi=300,bbox_inches='tight')
    print('fig saved')
