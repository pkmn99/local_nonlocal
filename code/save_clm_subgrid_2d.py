...
def convert_to_2d(df, var):
    ...
    for t in range(df.time.shape[0]):
        for i in range(1,17):
            ...
            temp[df.pfts1d_jxy[ind]-1, df.pfts1d_ixy[ind]-1] = df[var][0,ind]
            a[t,i,:,:]=temp
...
def save_to_2d(fn_in, fn_out):
    dc2=xr.open_dataset(fn_in, decode_times=False)
...
if __name__=="__main__":
    exp='F2000climo_ctl'
    fn_in='../data/outputdata/%s.clm2.h1.0001-0035-yearmean.nc'%exp
    fn_out='../data/outputdata/%s.clm2.h1.0001-0035-yearmean-2d.nc'%exp
